;;; spatial-window-geometry-emd.el --- Transport-inspired mapping -*- lexical-binding: t; -*-
...
;;; Commentary:
;; Transport-inspired mapping: initial assignment by overlap, then
;; rebalance keys toward area-proportional targets by moving low-overlap
;; cells from surplus windows to deficit windows.
...
(defun spatial-window--rebalance-transport (final kbd-rows kbd-cols window-bounds)
  "Rebalance key distribution toward area-proportional targets."
  (let* ((total-cells (* kbd-rows kbd-cols))
         (areas (mapcar #'spatial-window--window-area window-bounds))
         (area-sum (apply #'+ areas))
         (targets (mapcar (lambda (a) (max 1 (round (* total-cells (/ a area-sum))))) areas))
         (counts (spatial-window--count-all-keys final kbd-rows kbd-cols))
         (max-iters 200))
    (while (> max-iters 0)
      ...
      (let ((changed nil))
        (dotimes (i (length window-bounds))
          (let* ((wb (nth i window-bounds))
                 (win (car wb))
                 (need (- (nth i targets) (gethash win counts 0))))
            (when (> need 0)
              (let ((best-row nil)
                    (best-col nil)
                ...
                (dotimes (row kbd-rows)
                  (dotimes (col kbd-cols)
                    (let ((owner (aref (aref final row) col)))
                      (when owner
                        (let* ((donor-idx (cl-position owner window-bounds :key #'car))
                               (donor-target (and donor-idx (nth donor-idx targets)))
                               (donor-count (gethash owner counts 0)))
                          (when (and donor-idx (> donor-count donor-target))
                            (let* ((ov-donor (spatial-window--cell-overlap
                                              row col kbd-rows kbd-cols
                                              (nth 1 (nth donor-idx window-bounds))
                                              (nth 2 (nth donor-idx window-bounds))
                                              (nth 3 (nth donor-idx window-bounds))
                                              (nth 4 (nth donor-idx window-bounds))))
                                   (ov-rec (spatial-window--cell-overlap
                                            row col kbd-rows kbd-cols
                                            (nth 1 wb) (nth 2 wb) (nth 3 wb) (nth 4 wb)))
                                   (delta (- ov-donor ov-rec)))
      ...
      (let* ((window-bounds (or window-bounds (spatial-window--window-bounds frame)))
        ...
        (if (> num-windows (* kbd-rows kbd-cols))
          ...
          (let ((final (spatial-window--assign-cells kbd-rows kbd-cols window-bounds)))
            ...
            (spatial-window--rebalance-transport
             final kbd-rows kbd-cols window-bounds)
...
(provide 'spatial-window-geometry-emd)

;;; spatial-window-geometry-emd.el ends here
